---
layout: default
title: Gallery - Privacy Policy
---

# Gallery - Privacy Policy

**Last Updated: April 6, 2026**

Gallery ("we," "us," or "our") operates the Gallery mobile application (the "App"). This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our App. Please read this policy carefully. If you do not agree with the terms of this Privacy Policy, please do not access the App.

## 1. Information We Collect

### Information You Provide Directly

**Exhibition and Gallery Data.** You can save exhibitions you want to attend, log galleries you have visited, and follow galleries and artists. All of this activity data is stored locally on your device using Apple's SwiftData framework. We do not upload your exhibition saves, visit logs, or follow lists to our servers or to third-party servers.

**Art Preferences.** During onboarding and through in-app interactions, you may indicate your preferred art mediums, styles, and neighborhoods. This preference data is stored locally on your device and is used to personalize your experience.

**Communications.** If you contact our support team, we collect the content of your messages and any information you provide.

### Information Collected Automatically

**Usage Data.** We collect information about how you interact with the App, including screens viewed, features used, actions taken, the dates and times of your visits, and session duration.

**Device Information.** We collect device type, operating system version, unique device identifiers, and general device settings such as language and timezone.

**Crash and Performance Data.** We collect crash logs, error reports, and performance metrics to diagnose and fix issues.

**Attribution Data.** We collect information about how you discovered and installed the App, including which advertisement or campaign led to your install.

### Information We Do NOT Collect

- We do not upload your exhibition saves, gallery visit logs, or follow lists to our servers or to any third-party servers.
- We do not collect precise geolocation data or track your real-time location.
- We do not access your contacts, microphone, or photos.
- We do not use device fingerprinting or advertising identifiers (IDFA) for tracking purposes beyond install attribution.
- We do not share personal data with third-party AI models or services.

## 2. How We Use Your Information

We use the information we collect to:

- Provide, operate, and maintain the App and its features, including exhibition discovery, gallery profiles, and artist profiles
- Fetch and display exhibition, gallery, and artist data from the Artsy API on your behalf
- Process your subscriptions and manage premium features
- Monitor and analyze usage trends and preferences to improve the App
- Measure the effectiveness of our marketing campaigns and understand how users discover the App
- Detect, prevent, and address technical issues, fraud, and abuse
- Communicate with you about updates, support requests, and promotional offers related to the App (with your consent where required by law)
- Comply with legal obligations

## 3. Artsy API

Gallery uses the Artsy API (artsy.net) to fetch exhibition listings, gallery profiles, and artist profiles. When you browse content in the App, requests are made to Artsy's servers to retrieve this public exhibition data. Artsy's privacy policy governs the data it processes in connection with these requests. Artsy may log IP addresses and other metadata associated with API requests as part of standard web server operations. We do not transmit your personal saved data, follow lists, or visit logs to Artsy.

## 4. Local Data Storage

Exhibition saves, gallery visit logs, follow lists, and art preferences in Gallery are stored locally on your device using Apple's SwiftData framework. This personal activity data never leaves your device. We do not maintain any server-side database of your curation or gallery activity.

## 5. Third-Party Services

We use the following third-party services, each of which has its own privacy policy governing the data it processes:

| Service | Purpose | Data Shared | Privacy Link |
|---------|---------|-------------|--------------|
| PostHog | Product analytics | Anonymous usage data, screen views, feature interactions, session data | [posthog.com/privacy](https://posthog.com/privacy) |
| Sentry | Crash reporting and error monitoring | Device info, crash logs, anonymized usage context | [sentry.io/privacy](https://sentry.io/privacy/) |
| AppsFlyer | Install attribution and marketing analytics | Device identifiers (where permitted), install source, in-app conversion events | [appsflyer.com/privacy](https://www.appsflyer.com/privacy-policy/) |
| Facebook SDK (Meta) | Advertising attribution and App Events | Install attribution data, anonymized conversion events | [facebook.com/privacy](https://www.facebook.com/privacy/explanation) |
| RevenueCat | Subscription and purchase management | Anonymized user ID, subscription status, transaction identifiers | [revenuecat.com/privacy](https://www.revenuecat.com/privacy) |
| Artsy API | Exhibition, gallery, and artist data | API request metadata (IP address, device info per standard web requests) | [artsy.net/privacy](https://www.artsy.net/privacy) |

We do not sell your personal data to any third party. We do not upload or share your exhibition saves, visit logs, or follow lists with advertising networks or third-party services.

All third-party service providers are contractually required to protect your data with safeguards equal to or greater than those described in this Privacy Policy.

## 6. Data Retention

We retain your personal data for as long as the App is installed or as needed to provide you services. Specifically:

- Exhibition saves, visit logs, and follow lists are stored on-device and never stored on our servers.
- Art preference data is stored on-device only.
- Analytics data is retained for up to 12 months.
- Crash and performance logs are retained for up to 90 days.
- Communication records (support emails) are retained for up to 24 months.

When you delete the App, all locally stored data is removed from your device.

## 7. Data Security

We implement reasonable administrative, technical, and physical safeguards to protect your personal data, including:

- All exhibition and activity data is stored locally on your device — it never leaves your device
- All data stored locally using iOS system-level encryption
- No server-side storage of your personal exhibition logs or follow lists

No method of electronic transmission or storage is 100% secure. While we strive to protect your personal data, we cannot guarantee its absolute security.

## 8. Your Rights and Choices

Depending on your location, you may have the following rights:

- **Access.** Your activity data is stored on your device and accessible to you directly within the App at all times.
- **Deletion.** Deleting the App removes all locally stored data.
- **Opt Out of Analytics.** You can limit analytics collection by disabling "Allow Apps to Request to Track" in iOS Settings > Privacy & Security > Tracking.
- **Push Notifications.** You can disable push notifications at any time in iOS Settings > Notifications > Gallery.

To exercise any of these rights or with questions, please contact us at ben@thefoghill.com. We will respond within 30 days.

## 9. Children's Privacy

The App is not directed to children under the age of 13. We do not knowingly collect personal data from children under 13. If we learn that we have collected personal data from a child under 13, we will take steps to delete that data promptly. If you believe that a child under 13 has provided us with personal data, please contact us at ben@thefoghill.com.

## 10. International Data Transfers

Your data may be processed in the United States or other countries where our service providers operate. By using the App, you consent to the transfer of your data to these jurisdictions, which may have data protection laws that differ from those in your country.

## 11. California Privacy Rights (CCPA/CPRA)

If you are a California resident, you have additional rights under the California Consumer Privacy Act (CCPA) and the California Privacy Rights Act (CPRA):

- The right to know what personal data we collect, use, and disclose
- The right to request deletion of your personal data
- The right to opt out of the sale of personal data — we do not sell your personal data
- The right to non-discrimination for exercising your privacy rights

Categories of personal information we collect: identifiers (device ID), internet or other electronic network activity (usage data, crash logs), and commercial information (purchase/subscription records if applicable).

To exercise these rights, contact us at ben@thefoghill.com.

## 12. European Privacy Rights (GDPR)

If you are located in the European Economic Area (EEA) or United Kingdom, we process your data under the following legal bases:

- **Contract performance:** To provide the App and its features as described in our Terms of Service
- **Legitimate interests:** To improve the App, prevent fraud, and ensure security
- **Consent:** For optional features like push notifications

Under the GDPR, you have the right to:

- Access the personal data we hold about you
- Rectify inaccurate personal data
- Request erasure of your personal data
- Restrict or object to the processing of your personal data
- Data portability — receive your data in a structured, machine-readable format
- Withdraw consent at any time where processing is based on consent

You have the right to lodge a complaint with your local data protection authority if you believe your rights have been violated.

To exercise these rights, contact us at ben@thefoghill.com. We will respond to verified requests within 30 days.

## 13. Changes to This Privacy Policy

We may update this Privacy Policy from time to time. We will notify you of material changes by posting the updated policy in the App and updating the "Last Updated" date. Your continued use of the App after any changes constitutes your acceptance of the revised policy.

## 14. Contact Us

If you have questions about this Privacy Policy, please contact us:

**Email:** ben@thefoghill.com  
**Company:** Fog Hill LLC (d/b/a Foggy Labs)

This Privacy Policy is effective as of April 6, 2026.
